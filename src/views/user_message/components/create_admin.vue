<template>
    <el-dialog v-model="state.diagolFormVisible" :title="tilte" width="500px" align-center draggable>
        <div class="dialog-content">
            <el-form ref="ruleFormRef" :model="formData" :rules="rules" label-width="100px">
                <el-form-item label="账号" prop="account">
                    <el-input v-model="formData.account" />
                </el-form-item>
                <el-form-item label="密码" prop="password">
                    <el-input v-model="formData.password" />
                </el-form-item>
                <el-form-item label="姓名" prop="name">
                    <el-input v-model="formData.name" />
                </el-form-item>
                <el-form-item label="性别" prop="sex">
                    <el-select v-model="formData.sex" placeholder="请选择性别">
                        <el-option label="男" value="男" />
                        <el-option label="女" value="女" />
                    </el-select>
                </el-form-item>
                <el-form-item label="邮箱" prop="email">
                    <el-input v-model="formData.email" />
                </el-form-item>
                <el-form-item label="部门" prop="department">
                    <el-select v-model="formData.department" placeholder="请选择部门">
                        <!-- <el-option v-for="item in formData" :key="item" :label="item" :value="item" /> -->
                        <el-option label="项目部" value="项目部" />
                        <el-option label="总裁办" value="总裁办" />
                    </el-select>
                </el-form-item>
            </el-form>
        </div>
        <template #footer>
            <div class="dialog-footer">
                <el-button type="primary">
                    确定
                </el-button>
            </div>
        </template>
    </el-dialog>
</template>
<script lang="ts" setup>
import { reactive, ref } from 'vue'
import { bus } from "@/utils/mitt.js"
import { updateproductManager } from '../../../api/home'
import { ElMessage } from 'element-plus';

bus.on('createId', (id: number) => {
    if (id == 1) {
        tilte.value = '新建产品管理员'
    }
    if (id == 2) {
        tilte.value = '新建用户管理员'
    }
    if (id == 3) {
        tilte.value = '新建消息管理员'
    }
})


const tilte = ref()
//表单数据
interface formData {
    account: number,
    password: string,
    name: string,
    sex: string,
    email: string,
    department: string,
}

const formData: formData = reactive({
    account: '',
    password: '',
    name: '',
    sex: '',
    email: '',
    department: '',
})
//表单规则
const rules = reactive({
    account: [
        { required: true, message: '请输入管理员的注册账号', trigger: 'blur' },
    ],
    password: [
        { required: true, message: '请输入管理员的注册密码', trigger: 'blur' },
    ],
    name: [
        { required: true, message: '请输入管理员的用户名', trigger: 'blur' },
    ],
    sex: [
        { required: true, message: '请选择管理员的性别', trigger: 'blur' },
    ],
    email: [
        { required: true, message: '请输入管理员的注册邮箱', trigger: 'blur' },
    ],
    department: [
        { required: true, message: '请选择管理员的注册部门', trigger: 'blur' },
    ],
})
//添加产品管理员
// const updateProductManager = () => {
//     updateproductManager(formData).then(res => {
//         if (res.code != 0) {
//             ElMessage({
//                 message: res.msg,
//                 type: 'error'
//             })
//             return;
//         }
//         // localStorage.setItem('userinfo',JSON.stringify(res.data))
//         // console.log(res.data);
        
//         ElMessage.success("添加产品管理员成功")
//         state.diagolFormVisible = false;
//     })
// }

//弹窗开关
const state = reactive({
    diagolFormVisible: false,
    Product:JSON.parse(localStorage.getItem('userinfo'))
})
//打开创建管理员的弹窗
const open = () => {
    state.diagolFormVisible = true
}
defineExpose({
    open
})
</script>
<style lang="scss" scoped>
.dialog-content {
    display: flex;
    padding: 20px 30px;
}

:deep(.el-form-item) {
    margin: 20px;
}
</style>